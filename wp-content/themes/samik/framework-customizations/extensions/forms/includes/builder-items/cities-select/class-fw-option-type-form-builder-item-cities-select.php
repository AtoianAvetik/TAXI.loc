...
class FW_Option_Type_Form_Builder_Item_Cities_Select extends FW_Option_Type_Form_Builder_Item {
    ...
    public function get_thumbnails() {
        return array(
            array(
                'html' =>
                    '<div class="item-type-icon-title" data-hover-tip="' . __( 'Add an cities select', 'samik' ) . '">' .
                    '<div class="item-type-icon">' .
                    '<img src="' . esc_attr(  fw_get_template_customizations_directory_uri('/extensions/forms/includes/builder-items/cities-select/static/images/icon.png' ) ) . '" />' .
                    '</div>' .
                    '<div class="item-type-title">' . __( 'Cities Select', 'samik' ) . '</div>' .
                    '</div>'
            )
        );
    }
    ...
    public function enqueue_static() {
        $uri = fw_get_template_customizations_directory_uri('/extensions/forms/includes/builder-items/cities-select/static');
        ...
        wp_localize_script(
            'fw-builder-' . $this->get_builder_type() . '-item-' . $this->get_type(),
            'fw_form_builder_item_type_cities_select',
            array(
                'l10n'     => array(
                    'item_title'      => __( 'Cities Select', 'samik' ),
    ...
    private function get_options() {
        return array(
            ...
            array(
                'choices' => array(
                    'type'   => 'addable-option',
                    'value'  => get_option('cities_list_fw_settings_form')['choices'],
                    'label'  => __( 'Choices', 'samik' ),
                    'desc'   => __( 'Add choice', 'samik' ),
                    'option' => array(
                        'type' => 'text',
                    ),
                )
    ...
    protected function get_fixed_attributes( $attributes ) {
        ...
        if ( empty( $attributes['options']['choices'] ) ) {
            $attributes['options']['choices'][] = __( 'Dropdown', 'samik' );
        }
    ...
    public function frontend_render( array $item, $input_value ) {
        ...
        return fw_render_view(
            ...
            array(
                ...
                'attr'    => array(
                    'name' => $item['shortcode'],
                    'data-select' => $item['shortcode'] . fw_unique_increment(),
...
FW_Option_Type_Builder::register_item_type( 'FW_Option_Type_Form_Builder_Item_Cities_Select' );
